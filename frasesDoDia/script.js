...
function dataAtual() {
  ...
  return numero_escolhido; // Retornar o número sorteado para ser usado posteriormente
}

function imprimeFrase() {
  let fraseDoDia = frasesDoDia(); // Chamar a função frasesDoDia para obter a lista de frases
  let numeroSorteado = dataAtual(); // Chamar a função dataAtual para obter o número sorteado

  // Usar o número sorteado como índice para escolher uma frase aleatória
  let fraseEscolhida = fraseDoDia[numeroSorteado % fraseDoDia.length];

  console.log("Frase do dia:");
  console.log("Mensagem: " + fraseEscolhida);
}

// Chame a função imprimeFrase para exibir a frase do dia
imprimeFrase();
